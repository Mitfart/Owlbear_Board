...
- Before changing behavior, read root `CONTEXT.md` and relevant `docs/adr/`. Store complete board records in extension-owned hidden Scene Data Items; keep room metadata to compact board references and tombstones only. The per-item capacity is unpublished: never claim a 200 MB board-data limit. Shared edits are last-write-wins and live updates use polling.
