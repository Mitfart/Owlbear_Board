...
- For visual defects, trace the DOM and CSS layout chain before changing styles. Use explicitly themed, rounded interactive controls and installed icon components; keep matching controls visually consistent across the board and Markdown Help. In the text editor, keep the textarea and preview as equal-height panes; Markdown help toggles from `?`, closes on outside click or Escape, and scrolls inside its single dashed border.
