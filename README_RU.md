...
Данные досок хранятся в метаданных Owlbear: доски сцен, включая приватные, хранятся в метаданных сцены. Для досок комнаты метаданные комнаты служат главным реестром и переносятся в готовые сцены. В метаданных игрока остаются только идентификаторы активных досок комнаты текущего игрока. Вне Owlbear Rodeo localStorage используется как запасное хранилище. Для общих досок сохраняется последняя версия, поэтому договоритесь с группой, если одновременно редактируете один элемент.
