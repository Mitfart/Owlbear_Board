...
- **Надёжное хранение досок** — доски сцен сохраняются вместе со сценой; доски комнаты используют реестр на уровне комнаты и остаются доступными при смене сцен.
- **Понятный доступ к доскам** — приватной доской можно поделиться с выбранными игроками; ГМ всегда имеет доступ, а удалить доску может только создатель или ГМ.
- **Актуальные списки досок** — сохранение, изменение и удаление досок обновляют меню досок и окно управления у подключённых клиентов.
- **Улучшенное редактирование** — доработаны справка и форматирование Markdown, адаптивные текстовые элементы, числовой ввод, размещение и изменение размера досок.
- **Локальный буфер элементов доски** — скопируйте один элемент, переключитесь на другую доску и вставьте независимую копию с текстом, изображением, счётчиком, размерами и оформлением.
- **Быстрая работа с доской** — используйте Copy/Paste в контекстном меню или Ctrl/Cmd+C и Ctrl/Cmd+V; вставка с клавиатуры появляется в центре видимой области доски.
- **Более понятная обратная связь** — улучшены выделение элемента, перетаскивание с учётом столкновений и оформление карточек с изображениями.
...
Каждая доска сцены хранится в скрытом объекте Scene Data Item, принадлежащем расширению. Данные доски комнаты следуют за комнатой между готовыми сценами, а метаданные игрока содержат только пользовательские настройки и порядок открытых досок. Резервного хранения в браузере нет. Для общих досок сохраняется последняя версия, поэтому договоритесь с группой, если одновременно редактируете один элемент.
