...
function ColorPicker({ value, onChange, disabled = false }: { value: string; onChange: (value: string) => void; disabled?: boolean }) { return <input className="owlbearColorPicker" type="color" value={value} disabled={disabled} onChange={(event) => onChange(event.target.value)} />; }
...
export default function App() {
  ...
  async function createTextAt(target: { x: number; y: number }) {
    ...
    const item: BoardItem = { ...createItemBase(position.x, position.y, gridWidth, gridHeight), type: "text", text: "", textBaselineWidth: autoTextSize("").width, fontSize: 16, fillBlock: true, textVerticalAlignment: "top", borderColor: borderColorDraft };
  ...
  function openItemEditor(item: BoardItem) {
    ...
    if (item.type === "text") { setFocusDraft(item.text ?? ""); setTextFontSize(item.fontSize ?? 16); setTextFillBlock(item.fillBlock !== false); setTextVerticalAlignment(item.textVerticalAlignment ?? "top"); setHorizontalAlignmentOpen(false); setVerticalAlignmentOpen(false); setHasTextSelection(false); setImageEdit(undefined); setCounterEdit(undefined); }
  ...
  function toggleTextTask(item: BoardItem, line: number) {
    ...
    counterChangeQueue.current = counterChangeQueue.current.then(async () => {
      try {
        const draft = boardDraft.current; if (!draft) return; const saved = await saveBoard(draft);
        const current = boardDraft.current; if (current?.id === saved.id) {
          const revised = { ...current, revision: saved.revision, updatedAt: saved.updatedAt }; boardDraft.current = revised;
          setBoards((boards) => boards.map((candidate) => candidate.id === revised.id ? revised : candidate));
        }
