...
function ManageBoards({ boards, activeBoardId, onCreate, onCreateShared, onOpen, onSettings }: { boards: Board[]; activeBoardId?: string; onCreate(): void; onCreateShared(scope: BoardScope): void; onOpen(board: Board): void; onSettings(board: Board, event: React.PointerEvent<HTMLElement>): void }) {
  ...
  return <section className="manageBoards"><div className="manageBoardActions"><button className="primaryAction" onClick={onCreate}><Plus size={16} /> Create Private Board</button>{!boards.some((board) => board.visibility === "shared" && board.scope === "scene") && <button onClick={() => onCreateShared("scene")}>Shared Scene Board</button>}{!boards.some((board) => board.visibility === "shared" && board.scope === "room") && <button onClick={() => onCreateShared("room")}>Shared Room Board</button>}</div>{ordered.map(([owner, owned]) => <section key={owner}><h3>{owner}</h3><div className="manageBoardGrid">{owned.sort((a, b) => b.updatedAt.localeCompare(a.updatedAt)).map((board) => <button key={board.id} className="manageBoard" onClick={() => onOpen(board)}><span className="manageBoardDate">{date(board.updatedAt)}</span><strong>{board.name}</strong>{board.id === activeBoardId && <span className="manageBoardSettings" role="button" tabIndex={0} onPointerDown={(event) => { event.preventDefault(); event.stopPropagation(); onSettings(board, event); }}><Settings size={15} /></span>}</button>)}</div></section>)}</section>;
}
