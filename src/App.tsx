...
export default function App() {
  ...
  return <main className="app" style={{ width: windowSize.width, height: windowSize.height, ...themeVars }}>
    ...
    {focusedItem?.type === "text" && <div className="modalBackdrop" onPointerDown={() => void saveFocusedText()}><section className="editModal textEditModal" role="dialog" aria-modal="true" aria-labelledby="text-edit-title" onPointerDown={(event) => event.stopPropagation()}><div className="modalHeader"><div className="editTextTitle"><div className="markdownHelpTrigger"><button className="markdownHelpButton" aria-label="Markdown help">?</button><MarkdownHelp /></div><strong id="text-edit-title">Edit text</strong></div><div className="modalHeaderActions"><button className="primaryAction" onClick={() => void saveFocusedText()}><Save size={16} /> Save</button><button title="Cancel" onClick={() => void cancelFocusedText()}><X size={16} /></button></div></div><div className="editModalBody textEditLayout"><div className="textEditToolbar"><button title="Bold selected text" disabled={!hasTextSelection} onMouseDown={(event) => event.preventDefault()} onClick={() => toggleTextStyle("**")}><Bold size={16} /></button><button title="Italic selected text" disabled={!hasTextSelection} onMouseDown={(event) => event.preventDefault()} onClick={() => toggleTextStyle("*")}><Italic size={16} /></button><div className="alignmentDropdown"><button className="alignmentButton" title="Horizontal text alignment" onClick={() => setHorizontalAlignmentOpen((open) => !open)}>{[<AlignLeft size={16} />, <AlignCenter size={16} />, <AlignRight size={16} />, <AlignJustify size={16} />][preferences?.textAlignment ?? 0]}<ChevronDown size={13} /></button>{horizontalAlignmentOpen && <div className="alignmentMenu">{[[0, <AlignLeft size={16} />, "Align left"], [1, <AlignCenter size={16} />, "Align center"], [2, <AlignRight size={16} />, "Align right"], [3, <AlignJustify size={16} />, "Justify"]].map(([alignment, icon, label]) => <button key={String(alignment)} title={String(label)} onMouseDown={(event) => event.preventDefault()} onClick={() => { setHorizontalAlignmentOpen(false); void alignTextBlocks(Number(alignment) as 0 | 1 | 2 | 3); }}>{icon}</button>)}</div>}</div></div><div className="textEditControls"><div className="textPresentationOptions"><button className={`fillBlockButton ${textFillBlock ? "active" : ""}`} onClick={() => setTextFillBlock((value) => !value)}>Fill Block</button><div className="verticalAlignment"><span>Vertical alignment</span><div className="verticalAlignmentButtons">{[["top", <AlignVerticalJustifyStart size={16} />], ["center", <AlignVerticalJustifyCenter size={16} />], ["bottom", <AlignVerticalJustifyEnd size={16} />]].map(([alignment, icon]) => <button key={String(alignment)} className={textVerticalAlignment === alignment ? "active" : undefined} title={`Align ${String(alignment)}`} onClick={() => setTextVerticalAlignment(alignment as NonNullable<BoardItem["textVerticalAlignment"]>)}>{icon}</button>)}</div></div></div><textarea ref={focusTextarea} className="editorTextarea" value={focusDraft} autoFocus onScroll={(event) => { if (previewContent.current) syncTextScroll(event.currentTarget, previewContent.current); }} onChange={(event) => { setFocusDraft(event.target.value); setHasTextSelection(event.currentTarget.selectionStart !== event.currentTarget.selectionEnd); }} onSelect={(event) => setHasTextSelection(event.currentTarget.selectionStart !== event.currentTarget.selectionEnd)} onKeyUp={(event) => setHasTextSelection(event.currentTarget.selectionStart !== event.currentTarget.selectionEnd)} onKeyDown={(event) => { if (event.key === "Escape") { event.preventDefault(); void cancelFocusedText(); } if ((event.ctrlKey || event.metaKey) && event.key === "Enter") { event.preventDefault(); void saveFocusedText(); } }} /></div><aside className="textPreviewPane"><div className="textLivePreview"><div ref={previewContent} className="textPreviewContent" onScroll={(event) => { if (focusTextarea.current) syncTextScroll(event.currentTarget, focusTextarea.current); }}>{!focusDraft && <span className="previewPlaceholder">Preview</span>}<MarkdownView value={focusDraft} /></div></div></aside></div></section></div>}
