...
function ManageBoards({ boards, onOpen }: { boards: Board[]; onOpen(board: Board): void }) {
  const groups = new Map<string, Board[]>();
  for (const board of boards) { const owner = board.ownerName || board.ownerId || "Shared boards"; groups.set(owner, [...(groups.get(owner) ?? []), board]); }
  return <section className="manageBoards"><div className="debugHeader"><strong>Manage Boards</strong><span>{boards.length} boards</span></div>{[...groups].sort(([a], [b]) => a.localeCompare(b)).map(([owner, owned]) => <section key={owner}><h3>{owner}</h3><div className="manageBoardGrid">{owned.sort((a, b) => b.updatedAt.localeCompare(a.updatedAt)).map((board) => <button key={board.id} onClick={() => onOpen(board)}><strong>{board.name}</strong><span>{new Date(board.updatedAt).toLocaleString()}</span></button>)}</div></section>)}</section>;
}
