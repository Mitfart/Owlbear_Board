...
export default function App() {
  ...
  const canPickOwlbearImages = OBR.isAvailable && !/^temp/i.test(playerId);
  ...
  return <main className="app" style={{ width: windowSize.width, height: windowSize.height, ...themeVars }}>
    ...
    {manageBoardsOpen && <ManageBoards boards={boards} activeBoardId={activeBoardId} onCreate={() => setCreateOpen(true)} onCreateShared={(scope) => void createShared(scope)} onOpen={(board) => void chooseBoard(board)} onSettings={(board, event) => { event.stopPropagation(); void chooseBoard(board, false); const rect = event.currentTarget.getBoundingClientRect(); const x = rect.right + 8 + 360 <= window.innerWidth ? rect.right + 8 : Math.max(8, rect.left - 368); setBoardPanelPosition({ x, y: rect.top }); setBoardPanelOpen(true); }} />}
