...
import { AlignCenter, AlignJustify, AlignLeft, AlignRight, AlignVerticalJustifyCenter, AlignVerticalJustifyEnd, AlignVerticalJustifyStart, Bold, Check, ChevronDown, CircleAlert, ClipboardPaste, Copy, Grip, ImagePlus, Italic, Maximize2, Minus, PanelsTopLeft, Pencil, Plus, Save, Settings, Trash2, Type, X } from "lucide-react";
...
export default function App() {
  ...
  function copyBoardItem(item: BoardItem) {
    if (saveClipboard(item)) {
      setContextItem(undefined);
      return;
    }
    const message = "Could not copy Board Item to the local Clipboard.";
    setError(message); logDebug(message);
    if (OBR.isAvailable) void OBR.notification.show(message, "ERROR");
  }

  function copySelectedBoardItem() {
  ...
  function handleGridPointerDown(event: React.PointerEvent<HTMLDivElement>) {
    ...
    if (readOnly) { if (item) { setSelectedItemId(item.id); return; } if (event.button === 0) setPanning({ x: event.clientX - pan.x, y: event.clientY - pan.y }); event.currentTarget.setPointerCapture(event.pointerId); return; }
  ...
  return <ColorPickerPreferences.Provider value={{ paletteColors: paletteForSlots(colorPalette), onAddColor: addPaletteColor, onUpdateColor: updatePaletteColor, onDeleteColor: deletePaletteColor }}><main className="app" style={{ width: windowSize.width, height: windowSize.height, ...themeVars }}>
    ...
    {contextItem && <div className="contextMenu" style={{ left: contextItem.x, top: contextItem.y }}>{!readOnly && <button onClick={() => { openItemEditor(contextItem.item); setContextItem(undefined); }}><Pencil size={15} /> Edit</button>}<button onClick={() => copyBoardItem(contextItem.item)}><Copy size={15} /> Copy</button>{!readOnly && <><button disabled={!loadClipboard()} onClick={() => void pasteClipboardAt(pointerToGrid(contextItem.x, contextItem.y))}><ClipboardPaste size={15} /> Paste</button><label className="colorMenuItem">Border<ColorPicker value={contextItem.item.borderColor ?? DEFAULT_ITEM_BORDER_COLOR} defaultColor={DEFAULT_ITEM_BORDER_COLOR} onChange={(borderColor) => void updateContextItemBorder(borderColor)} /></label><button onClick={() => void deleteItem(contextItem.item.id)}><Trash2 size={15} /> Delete</button></>}</div>}
    {emptyContext && <div className="contextMenu creationMenu" style={{ left: emptyContext.x, top: emptyContext.y }}><button disabled={!activeBoard?.items.some((item) => item.id === selectedItemId)} onClick={copySelectedBoardItem}><Copy size={15} /> Copy</button><button disabled={!loadClipboard()} onClick={() => void pasteClipboardAt({ x: emptyContext.gridX, y: emptyContext.gridY })}><ClipboardPaste size={15} /> Paste</button><button onClick={() => { void createTextAt({ x: emptyContext.gridX, y: emptyContext.gridY }); setEmptyContext(undefined); }}><Type size={15} /> Add Text</button><button onClick={() => { setAddTarget({ x: emptyContext.gridX, y: emptyContext.gridY }); setAddItemType("counter"); setAddModalOpen(true); setEmptyContext(undefined); }}><Plus size={15} /> Add Counter</button><button onClick={() => { setAddTarget({ x: emptyContext.gridX, y: emptyContext.gridY }); setAddItemType("image"); setAddModalOpen(true); setEmptyContext(undefined); }}><ImagePlus size={15} /> Add Image</button></div>}
