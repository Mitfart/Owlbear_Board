...
import { autoImageSize, autoTextSize, clampNumber, normalizeCounterValue, parseItemSize } from "./sizing";
...
type CounterEdit = { itemId: string; label: string; value: string; max: string; hasMax: boolean; borderColor: string; zeroColorEnabled: boolean; zeroColor: string; maxColorEnabled: boolean; maxColor: string; dimAtZero: boolean };
...
export default function App() {
  ...
  async function addCounter() {
    if (!activeBoard) return;
    const target = addTarget ?? viewportCenterGrid(); const position = firstFreeNear(activeBoard, target.x, target.y, 2, 1);
    const item: BoardItem = { ...createItemBase(position.x, position.y, 2, 1), type: "counter", counterValue: 0, counterDimAtZero: true, borderColor: borderColorDraft, counterZeroColor: DEFAULT_COUNTER_ZERO_COLOR, counterMaxColor: DEFAULT_COUNTER_MAX_COLOR };
    await persistBoard({ ...activeBoard, items: [...activeBoard.items, item] }); setAddModalOpen(false); setAddTarget(undefined);
  }
  ...
  function openItemEditor(item: BoardItem) {
    ...
    if (item.type === "text") { setFocusDraft(item.text ?? ""); setHasTextSelection(false); setImageEdit(undefined); setCounterEdit(undefined); }
    ...
    else setCounterEdit({ itemId: item.id, label: item.counterLabel ?? "", value: String(item.counterValue ?? 0), max: item.counterMax === undefined ? "" : String(item.counterMax), hasMax: item.counterMax !== undefined, borderColor: item.borderColor ?? DEFAULT_ITEM_BORDER_COLOR, zeroColorEnabled: !!item.counterZeroColorEnabled, zeroColor: item.counterZeroColor ?? DEFAULT_COUNTER_ZERO_COLOR, maxColorEnabled: !!item.counterMaxColorEnabled, maxColor: item.counterMaxColor ?? DEFAULT_COUNTER_MAX_COLOR, dimAtZero: item.counterDimAtZero ?? true });
  }
  ...
  async function changeCounter(item: BoardItem, delta: number) {
    if (!activeBoard) return;
    const value = normalizeCounterValue((item.counterValue ?? 0) + delta, item.counterMax);
    if (value === item.counterValue) return;
    await persistBoard({ ...activeBoard, items: activeBoard.items.map((candidate) => candidate.id === item.id ? { ...candidate, counterValue: value, updatedAt: nowIso() } : candidate) });
  }
  ...
  return <main className="app" style={{ width: windowSize.width, height: windowSize.height, ...themeVars }}>
    ...
    {focusedItem?.type === "text" && <div className="modalBackdrop" onPointerDown={() => void saveFocusedText()}><section className="editModal textEditModal" onPointerDown={(event) => event.stopPropagation()}><div className="modalHeader"><strong>Edit text</strong><div className="modalHeaderActions"><button className="primaryAction" onClick={() => void saveFocusedText()}><Save size={16} /> Save</button><button title="Cancel" onClick={() => void cancelFocusedText()}><X size={16} /></button></div></div><div className="editModalBody textEditLayout"><div className="textEditToolbar"><button title="Bold selected text" disabled={!hasTextSelection} onMouseDown={(event) => event.preventDefault()} onClick={() => toggleTextStyle("**")}><Bold size={16} /></button><button title="Italic selected text" disabled={!hasTextSelection} onMouseDown={(event) => event.preventDefault()} onClick={() => toggleTextStyle("*")}><Italic size={16} /></button><button title="Align left" onMouseDown={(event) => event.preventDefault()} onClick={() => void alignTextBlocks(0)}><AlignLeft size={16} /></button><button title="Align center" onMouseDown={(event) => event.preventDefault()} onClick={() => void alignTextBlocks(1)}><AlignCenter size={16} /></button><button title="Align right" onMouseDown={(event) => event.preventDefault()} onClick={() => void alignTextBlocks(2)}><AlignRight size={16} /></button><button title="Justify" onMouseDown={(event) => event.preventDefault()} onClick={() => void alignTextBlocks(3)}><AlignJustify size={16} /></button></div><div className="textEditControls"><textarea ref={focusTextarea} className="editorTextarea" value={focusDraft} autoFocus onChange={(event) => { setFocusDraft(event.target.value); setHasTextSelection(event.currentTarget.selectionStart !== event.currentTarget.selectionEnd); }} onSelect={(event) => setHasTextSelection(event.currentTarget.selectionStart !== event.currentTarget.selectionEnd)} onKeyUp={(event) => setHasTextSelection(event.currentTarget.selectionStart !== event.currentTarget.selectionEnd)} onKeyDown={(event) => { if (event.key === "Escape") { event.preventDefault(); void cancelFocusedText(); } if ((event.ctrlKey || event.metaKey) && event.key === "Enter") { event.preventDefault(); void saveFocusedText(); } }} /></div><aside className="textPreviewPane"><div className="textLivePreview">{!focusDraft && <span className="previewPlaceholder">Preview</span>}<div className="textPreviewContent"><MarkdownView value={focusDraft} /></div></div></aside></div></section></div>}
    ...
    {counterEdit && <div className="modalBackdrop" onPointerDown={() => void saveFocusedCounter()}><section className="editModal" onPointerDown={(event) => event.stopPropagation()}><div className="modalHeader"><strong>Edit counter</strong><div className="modalHeaderActions"><button className="primaryAction" onClick={() => void saveFocusedCounter()}><Save size={16} /> Save</button><button title="Cancel" onClick={cancelFocusedCounter}><X size={16} /></button></div></div><div className="editModalBody"><label>Label<input value={counterEdit.label} maxLength={120} onChange={(event) => setCounterEdit({ ...counterEdit, label: event.target.value })} /></label><div className="counterEditNumbers"><label>Value<input type="number" min="0" step="1" value={counterEdit.value} onChange={(event) => setCounterEdit({ ...counterEdit, value: event.target.value })} /></label><label><span>Maximum</span><input type="checkbox" checked={counterEdit.hasMax} onChange={(event) => setCounterEdit({ ...counterEdit, hasMax: event.target.checked })} /><input type="number" min="0" step="1" disabled={!counterEdit.hasMax} value={counterEdit.max} onChange={(event) => setCounterEdit({ ...counterEdit, max: event.target.value })} /></label></div><label className="imageOption"><span>Border</span><input type="color" value={counterEdit.borderColor} onChange={(event) => setCounterEdit({ ...counterEdit, borderColor: event.target.value })} /></label><label className="counterOption"><input type="checkbox" checked={counterEdit.zeroColorEnabled} onChange={(event) => setCounterEdit({ ...counterEdit, zeroColorEnabled: event.target.checked })} /> Zero border color <input type="color" disabled={!counterEdit.zeroColorEnabled} value={counterEdit.zeroColor} onChange={(event) => setCounterEdit({ ...counterEdit, zeroColor: event.target.value })} /></label><label className="counterOption"><input type="checkbox" checked={counterEdit.maxColorEnabled} onChange={(event) => setCounterEdit({ ...counterEdit, maxColorEnabled: event.target.checked })} /> Maximum border color <input type="color" disabled={!counterEdit.maxColorEnabled} value={counterEdit.maxColor} onChange={(event) => setCounterEdit({ ...counterEdit, maxColor: event.target.value })} /></label><label className="counterOption"><input type="checkbox" checked={counterEdit.dimAtZero} onChange={(event) => setCounterEdit({ ...counterEdit, dimAtZero: event.target.checked })} /> Dim at zero</label></div></section></div>}
...
function BoardItemView({ item, selected, cellSize, cellGap, onResizePointerDown, onDoubleClick, onCounterChange }: { item: BoardItem; selected: boolean; cellSize: number; cellGap: number; onResizePointerDown: (event: React.PointerEvent<HTMLElement>, item: BoardItem) => void; onDoubleClick: (item: BoardItem) => void; onCounterChange: (item: BoardItem, delta: number) => void }) {
  ...
  const value = item.counterValue ?? 0;
  const atMax = item.type === "counter" && item.counterMax !== undefined && value === item.counterMax;
  ...
  return <article className={`boardItem ${item.type} ${selected ? "selected" : ""} ${item.type === "counter" && value === 0 && item.counterDimAtZero !== false ? "zeroDim" : ""}`} style={{ left: item.gridX * cellSize + inset, top: item.gridY * cellSize + inset, width: Math.max(8, item.gridWidth * cellSize - inset * 2), height: Math.max(8, item.gridHeight * cellSize - inset * 2), borderColor }} onMouseDown={(event) => { if (event.detail === 2) { event.stopPropagation(); onDoubleClick(item); } }} onDoubleClick={(event) => { event.stopPropagation(); onDoubleClick(item); }}>
    {item.type === "image" && item.imageUrl ? <img src={item.imageUrl} alt="Board item" style={{ objectFit: item.imageFit ?? "cover" }} /> : item.type === "counter" ? <><button className="counterControl" aria-label="Decrease counter" disabled={value === 0} onPointerDown={(event) => event.stopPropagation()} onClick={(event) => { event.stopPropagation(); void onCounterChange(item, -1); }}><Minus size={16} /></button><div className="counterContent">{item.counterLabel && <span className="counterLabel" title={item.counterLabel}>{item.counterLabel}</span>}<div className="counterNumbers"><strong>{value}</strong>{item.counterMax !== undefined && <span>/ {item.counterMax}</span>}</div></div><button className="counterControl" aria-label="Increase counter" disabled={atMax} onPointerDown={(event) => event.stopPropagation()} onClick={(event) => { event.stopPropagation(); void onCounterChange(item, 1); }}><Plus size={16} /></button></> : <div className="textPreview"><MarkdownView value={item.text || ""} /></div>}
