...
export default function App() {
  ...
  return <ColorPickerPreferences.Provider value={{ paletteColors: paletteForSlots(colorPalette), onAddColor: addPaletteColor, onUpdateColor: updatePaletteColor, onDeleteColor: deletePaletteColor }}><main className="app" style={{ width: windowSize.width, height: windowSize.height, ...themeVars }}>
    ...
    {addModalOpen && <div className="modalBackdrop" onPointerDown={() => setAddModalOpen(false)}><section className="addModal" onPointerDown={(event) => event.stopPropagation()}><div className="modalHeader"><strong>Add item</strong><button onClick={() => setAddModalOpen(false)}><Minus size={16} /></button></div><div className="itemTypeTabs"><button className={addItemType === "text" ? "active" : undefined} onClick={() => setAddItemType("text")}><Type size={16} /> Text</button><button className={addItemType === "image" ? "active" : undefined} onClick={() => setAddItemType("image")}><ImagePlus size={16} /> Image</button><button className={addItemType === "counter" ? "active" : undefined} onClick={() => setAddItemType("counter")}><Plus size={16} /> Counter</button></div><div className="modalGrid"><label>W<input value={itemWidth} onChange={(event) => setItemWidth(event.target.value)} /></label><label>H<input value={itemHeight} onChange={(event) => setItemHeight(event.target.value)} /></label><label>Border<ColorPicker value={borderColorDraft} defaultColor={DEFAULT_ITEM_BORDER_COLOR} onChange={setBorderColorDraft} /></label>{addItemType === "text" ? <button className="primaryAction" onClick={() => { if (activeBoard) void createTextAt(addTarget ?? viewportCenterGrid()).then(() => setAddModalOpen(false)); }}><Type size={16} /> Add text</button> : addItemType === "counter" ? <><label className="counterAddField">Initial value<input type="number" min={counterMinDraft || "0"} max={counterMaxDraft || undefined} step="1" value={counterValueDraft} onChange={(event) => setCounterValueDraft(event.target.value)} /></label><label className="counterAddField">Minimum<input aria-label="Initial minimum value" type="number" min="0" max={counterMaxDraft || undefined} step="1" placeholder="no min" value={counterMinDraft} onChange={(event) => setCounterMinDraft(event.target.value)} /></label><label className="counterAddField">Maximum<input aria-label="Initial maximum value" type="number" min={counterMinDraft || "0"} step="1" placeholder="no max" value={counterMaxDraft} onChange={(event) => setCounterMaxDraft(event.target.value)} /></label><button className="primaryAction" onClick={() => void addCounter()}><Plus size={16} /> Add counter</button></> : <><label className="wideField">Image URL<input value={imageDraft} onChange={(event) => { setImageDraft(event.target.value); setImagePreviewSize(undefined); }} /></label>{canPickOwlbearImages && <button onClick={() => void pickOwlbearImage()}><ImagePlus size={16} /> Owlbear</button>}<button className="primaryAction" onClick={() => void addImage()}><ImagePlus size={16} /> Add</button>{imageDraft.trim() && <div className="imagePreviewPanel"><img src={imageDraft.trim()} alt="Image preview" onLoad={(event) => setImagePreviewSize({ width: event.currentTarget.naturalWidth, height: event.currentTarget.naturalHeight })} onError={() => setImagePreviewSize(undefined)} /></div>}</>}</div></section></div>}
...
function BoardItemView({ item, selected, editPresence, cellSize, cellGap, onResizePointerDown, onDoubleClick, onCounterChange, onTaskToggle, readOnly = false }: { item: BoardItem; selected: boolean; editPresence?: EditPresence; cellSize: number; cellGap: number; onResizePointerDown: (event: React.PointerEvent<HTMLElement>, item: BoardItem) => void; onDoubleClick: (item: BoardItem) => void; onCounterChange: (item: BoardItem, delta: number) => void; onTaskToggle: (item: BoardItem, line: number) => void; readOnly?: boolean }) {
  const inset = Math.min(cellGap, Math.max(0, (Math.min(item.gridWidth, item.gridHeight) * cellSize) / 2 - 4)); const value = item.counterValue ?? 0; const atMin = item.type === "counter" && value === (item.counterMin ?? 0); const atMax = item.type === "counter" && item.counterMax !== undefined && value === item.counterMax;
