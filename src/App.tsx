...
import { AlignCenter, AlignJustify, AlignLeft, AlignRight, AlignVerticalJustifyCenter, AlignVerticalJustifyEnd, AlignVerticalJustifyStart, Bold, ChevronDown, Grip, ImagePlus, Italic, Maximize2, Minus, PanelsTopLeft, Pencil, Plus, Save, Settings, Trash2, Type, X } from "lucide-react";
...
export default function App() {
  ...
  async function chooseBoard(board: Board) {
    ...
    clearBoardUi(); setActiveBoardId(board.id); setPan(viewport?.pan ?? DEFAULT_PAN); setZoom(viewport?.zoom ?? DEFAULT_ZOOM); setManageBoardsOpen(false); setOpenBoardIds((ids) => ids.includes(board.id) ? ids : [...ids, board.id]); await markPrivateBoardOpened(board);
  }
...
function ManageBoards({ boards, activeBoardId, onCreate, onOpen, onSettings }: { boards: Board[]; activeBoardId?: string; onCreate(): void; onOpen(board: Board): void; onSettings(board: Board): void }) {
  ...
  return <section className="manageBoards"><button className="primaryAction" onClick={onCreate}><Plus size={16} /> Create Private Board</button>{ordered.map(([owner, owned]) => <section key={owner}><h3>{owner}</h3><div className="manageBoardGrid">{owned.sort((a, b) => b.updatedAt.localeCompare(a.updatedAt)).map((board) => <button key={board.id} className="manageBoard" onClick={() => onOpen(board)}><span className="manageBoardDate">{date(board.updatedAt)}</span><strong>{board.name}</strong>{board.id === activeBoardId && <span className="manageBoardSettings" onClick={(event) => { event.stopPropagation(); onSettings(board); }}><Settings size={15} /></span>}</button>)}</div></section>)}</section>;
}
