...
export default function App() {
  ...
  return <main className="app" style={{ width: windowSize.width, height: windowSize.height, ...themeVars }}>
    ...
    {focusedItem?.type === "text" && <div className="modalBackdrop" onPointerDown={() => void saveFocusedText()}><section className="editModal textEditModal" onPointerDown={(event) => event.stopPropagation()}><div className="modalHeader"><strong>Edit text</strong><button title="Cancel" onClick={() => void cancelFocusedText()}><X size={16} /></button></div><div className="editModalBody textEditLayout"><div className="textEditControls"><div className="textEditToolbar"><button title="Bold selected text" disabled={!hasTextSelection} onMouseDown={(event) => event.preventDefault()} onClick={() => toggleTextStyle("**")}><Bold size={16} /></button><button title="Italic selected text" disabled={!hasTextSelection} onMouseDown={(event) => event.preventDefault()} onClick={() => toggleTextStyle("*")}><Italic size={16} /></button><button title="Align left" onMouseDown={(event) => event.preventDefault()} onClick={() => void alignTextBlocks(0)}><AlignLeft size={16} /></button><button title="Align center" onMouseDown={(event) => event.preventDefault()} onClick={() => void alignTextBlocks(1)}><AlignCenter size={16} /></button><button title="Align right" onMouseDown={(event) => event.preventDefault()} onClick={() => void alignTextBlocks(2)}><AlignRight size={16} /></button><button title="Justify" onMouseDown={(event) => event.preventDefault()} onClick={() => void alignTextBlocks(3)}><AlignJustify size={16} /></button></div><textarea ref={focusTextarea} className="editorTextarea" value={focusDraft} autoFocus onChange={(event) => { setFocusDraft(event.target.value); setHasTextSelection(event.currentTarget.selectionStart !== event.currentTarget.selectionEnd); }} onSelect={(event) => setHasTextSelection(event.currentTarget.selectionStart !== event.currentTarget.selectionEnd)} onKeyUp={(event) => setHasTextSelection(event.currentTarget.selectionStart !== event.currentTarget.selectionEnd)} onKeyDown={(event) => { if (event.key === "Escape") { event.preventDefault(); void cancelFocusedText(); } if ((event.ctrlKey || event.metaKey) && event.key === "Enter") { event.preventDefault(); void saveFocusedText(); } }} /><div className="editorActions"><button className="primaryAction" onClick={() => void saveFocusedText()}><Save size={16} /> Save</button></div></div><aside className="textLivePreview"><MarkdownView value={focusDraft} /></aside></div></section></div>}
    {imageEdit && <div className="modalBackdrop" onPointerDown={() => void saveFocusedImage()}><section className="editModal" onPointerDown={(event) => event.stopPropagation()} onKeyDown={(event) => { if (event.key === "Escape") { event.preventDefault(); cancelFocusedImage(); } if ((event.ctrlKey || event.metaKey) && event.key === "Enter") { event.preventDefault(); void saveFocusedImage(); } }}><div className="modalHeader"><strong>Edit image</strong><button title="Cancel" onClick={cancelFocusedImage}><X size={16} /></button></div><div className="editModalBody imageEditBody"><div className="imageUrlRow"><label>Image URL<input value={imageEdit.url} onChange={(event) => setImageEdit({ ...imageEdit, url: event.target.value })} /></label><button onClick={() => void pickOwlbearEditImage()} disabled={!OBR.isAvailable}><ImagePlus size={16} /> Owlbear</button></div><div className="imageOptionRow"><label className="imageOption"><span>Border</span><input type="color" value={imageEdit.borderColor} onChange={(event) => setImageEdit({ ...imageEdit, borderColor: event.target.value })} /></label><label className="imageOption"><span>Fit</span><select value={imageEdit.imageFit} onChange={(event) => setImageEdit({ ...imageEdit, imageFit: event.target.value as ImageEdit["imageFit"] })}><option value="cover">Cover</option><option value="contain">Contain</option></select></label></div><div className="editorActions"><button className="primaryAction fullSave" onClick={() => void saveFocusedImage()}><Save size={16} /> Save</button></div></div></section></div>}
