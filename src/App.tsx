...
export default function App() {
  ...
  return <main className="app" style={{ width: windowSize.width, height: windowSize.height, ...themeVars }}>
    ...
    {focusedItem?.type === "text" && <div className="modalBackdrop" onPointerDown={() => void saveFocusedText()}><section className="editModal textEditModal" onPointerDown={(event) => event.stopPropagation()}><div className="modalHeader"><strong>Edit text</strong><div className="modalHeaderActions"><button className="primaryAction" onClick={() => void saveFocusedText()}><Save size={16} /> Save</button><button title="Cancel" onClick={() => void cancelFocusedText()}><X size={16} /></button></div></div><div className="editModalBody textEditLayout"><div className="textEditToolbar"><button title="Bold selected text" disabled={!hasTextSelection} onMouseDown={(event) => event.preventDefault()} onClick={() => toggleTextStyle("**")}><Bold size={16} /></button><button title="Italic selected text" disabled={!hasTextSelection} onMouseDown={(event) => event.preventDefault()} onClick={() => toggleTextStyle("*")}><Italic size={16} /></button><button title="Align left" onMouseDown={(event) => event.preventDefault()} onClick={() => void alignTextBlocks(0)}><AlignLeft size={16} /></button><button title="Align center" onMouseDown={(event) => event.preventDefault()} onClick={() => void alignTextBlocks(1)}><AlignCenter size={16} /></button><button title="Align right" onMouseDown={(event) => event.preventDefault()} onClick={() => void alignTextBlocks(2)}><AlignRight size={16} /></button><button title="Justify" onMouseDown={(event) => event.preventDefault()} onClick={() => void alignTextBlocks(3)}><AlignJustify size={16} /></button><button title="Markdown help" aria-label="Markdown help" onClick={() => setMarkdownHelpOpen((open) => !open)}>Markdown help</button>{markdownHelpOpen && <MarkdownHelp />}</div><div className="textEditControls"><div className="textPresentationOptions"><label><input type="checkbox" checked={textFillBlock} onChange={(event) => setTextFillBlock(event.target.checked)} /> Fill Block</label><label>Vertical alignment<select value={textVerticalAlignment} onChange={(event) => setTextVerticalAlignment(event.target.value as NonNullable<BoardItem["textVerticalAlignment"]>)}><option value="top">Top</option><option value="center">Center</option><option value="bottom">Bottom</option></select></label></div><textarea ref={focusTextarea} className="editorTextarea" value={focusDraft} autoFocus onChange={(event) => { setFocusDraft(event.target.value); setHasTextSelection(event.currentTarget.selectionStart !== event.currentTarget.selectionEnd); }} onSelect={(event) => setHasTextSelection(event.currentTarget.selectionStart !== event.currentTarget.selectionEnd)} onKeyUp={(event) => setHasTextSelection(event.currentTarget.selectionStart !== event.currentTarget.selectionEnd)} onKeyDown={(event) => { if (event.key === "Escape") { event.preventDefault(); void cancelFocusedText(); } if ((event.ctrlKey || event.metaKey) && event.key === "Enter") { event.preventDefault(); void saveFocusedText(); } }} /></div><aside className="textPreviewPane"><div className="textLivePreview">{!focusDraft && <span className="previewPlaceholder">Preview</span>}<div className="textPreviewContent"><MarkdownView value={focusDraft} /></div></div></aside></div></section></div>}
