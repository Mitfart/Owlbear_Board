...
function ColorPicker({ value, onChange, disabled = false }: { value: string; onChange: (value: string) => void; disabled?: boolean }) {
  const [open, setOpen] = useState(false);
  return <span className="owlbearColorPicker"><button type="button" aria-label={`Color ${value}`} aria-expanded={open} disabled={disabled} onClick={() => setOpen((current) => !current)}><span style={{ backgroundColor: value }} /></button>{open && <div className="owlbearColorMenu" role="menu">{OWLBEAR_COLORS.map((color) => <button key={color} type="button" role="menuitemradio" aria-label={color} aria-checked={color.toLowerCase() === value.toLowerCase()} onClick={() => { onChange(color); setOpen(false); }}><span style={{ backgroundColor: color }} /></button>)}<label aria-label="Custom color"><Plus size={18} /><input type="color" value={value} onChange={(event) => { onChange(event.target.value); setOpen(false); }} /></label></div>}</span>;
}
