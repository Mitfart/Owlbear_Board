...
type CounterEdit = { itemId: string; label: string; labelPosition: NonNullable<BoardItem["counterLabelPosition"]>; value: string; max: string; hasMax: boolean; borderColor: string; zeroColorEnabled: boolean; zeroColor: string; maxColorEnabled: boolean; maxColor: string; dimAtZero: boolean };
...
export default function App() {
  ...
  const [counterHasMaxDraft, setCounterHasMaxDraft] = useState(false);
  ...
  async function addCounter() {
    ...
    const max = counterHasMaxDraft ? normalizeCounterValue(Number(counterMaxDraft)) : undefined;
    const item: BoardItem = { ...createItemBase(position.x, position.y, 2, 1), type: "counter", counterValue: normalizeCounterValue(Number(counterValueDraft), max), counterMax: max, counterLabelPosition: "top-center", counterDimAtZero: true, borderColor: borderColorDraft, counterZeroColor: DEFAULT_COUNTER_ZERO_COLOR, counterMaxColor: DEFAULT_COUNTER_MAX_COLOR };
    await persistBoard({ ...activeBoard, items: [...activeBoard.items, item] }); setAddModalOpen(false); setAddTarget(undefined); setCounterValueDraft("0"); setCounterMaxDraft(""); setCounterHasMaxDraft(false);
  }
