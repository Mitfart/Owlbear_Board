...
describe("board mutation coordinator", () => {
  ...
  it("serializes saves, reconciles revisions, rolls back failures, and owns history", async () => {
    ...
    const coordinator = createBoardMutationCoordinator({ save, apply: (next) => applied.push(`${next.id}:${next.revision}`), reportError: (error) => errors.push(error), reportDebug: vi.fn() });
